  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid border-l border-t border-[#DDE2DB] sm:grid-cols-2 xl:grid-cols-4">
        <Metric label="ReuniÃµes" value="14" change="+ 3 neste mÃªs" icon={CalendarDays} tone="green" />
        <Metric label="PresenÃ§a mÃ©dia" value="88%" change="Meta anual: 85%" icon={UsersRound} tone="sage" />
        <Metric label="ResoluÃ§Ãµes" value="36" change="7 aguardam publicaÃ§Ã£o" icon={BookOpen} tone="gold" />
        <Metric label="PendÃªncias" value="09" change="2 prazos vencem hoje" icon={ListChecks} tone="clay" />
      </section>
      ...
      <section className="grid gap-8 xl:grid-cols-[minmax(0,1.55fr)_minmax(300px,0.8fr)]">
        <article className="border-b border-[#DDE2DB] pb-1">
          <SectionHeading index="01" label="Ritmo institucional" title="ReuniÃµes realizadas" action={<button onClick={() => setLocation("/relatorios")} className="hidden items-center gap-1 text-[11px] font-bold text-[#285A43] sm:inline-flex">RelatÃ³rio completo <ArrowUpRight className="size-3" /></button>} />
...
function GenericModule({ active }: { active: ModuleKey }) {
  ...
  return <div className="grid min-h-[420px] place-items-center border border-[#DDE2DB] bg-[#FCFBF7] p-7 text-center"><div className="max-w-md"><div className="mx-auto grid size-12 place-items-center rounded-2xl bg-[#E8F0E8] text-[#285A43]"><Icon className="size-5" /></div><p className="mt-6 text-[10px] font-bold uppercase tracking-[0.15em] text-[#A9533A]">MÃ³dulo em preparaÃ§Ã£o</p><h2 className="mt-2 font-editorial text-[28px] font-semibold tracking-[-0.045em] text-[#193B32]">{data.title}</h2><p className="mt-3 text-[13px] leading-6 text-[#657268]">A estrutura de navegaÃ§Ã£o, permissÃµes e estados para este mÃ³dulo jÃ¡ estÃ¡ prevista no MVP. A prÃ³xima etapa conecta esta tela Ã  base Supabase e Ã s regras de acesso.</p><Button onClick={() => toast.message("MÃ³dulo registrado no roteiro de implementaÃ§Ã£o do MVP.")} variant="outline" className="mt-6 rounded-xl border-[#CBD4CA] bg-white text-[12px] font-bold text-[#285A43] hover:bg-[#EAF1E9]">Registrar prioridade <ArrowRight className="ml-2 size-3.5" /></Button></div></div>;
}
