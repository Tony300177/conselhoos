...
function StatusPill({ children, tone = "confirmed" }: { children: React.ReactNode; tone?: "confirmed" | "review" | "pending" | "neutral" | "private" | "danger" }) {
  return <span className={cn("inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-bold tracking-[0.02em] before:size-1.5 before:rounded-full", statusStyles[tone])}>{children}</span>;
}
  ...
  const inner = (
    <>
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-[0.13em] text-[#607068]">{label}</p>
  ...
  return (
    <div className="mb-4 flex items-end justify-between gap-4">
      <div className="flex items-end gap-3">
        ...
        <div><p className="text-[10px] font-bold uppercase tracking-[0.13em] text-[#607068]">{label}</p><h2 className="mt-1 font-editorial text-[23px] font-semibold tracking-[-0.035em] text-[#193B32]">{title}</h2></div>
      </div>
      {action}
    </div>
  );
...
function CivicRail({ active }: { active: ModuleKey }) {
  ...
  return <section className="mb-8 overflow-hidden border border-[#D5DDD4] bg-[#FCFBF7]"><div className="grid sm:grid-cols-[156px_1fr]"><div className="border-b border-[#244D42] bg-[#173F34] px-5 py-4 text-white sm:border-b-0 sm:border-r"><p className="text-[9px] font-bold uppercase tracking-[0.15em] text-[#BFD2C1]">RÃ©gua cÃ­vica</p><p className="mt-2 font-editorial text-[18px] font-semibold tracking-[-0.035em]">{protocol}</p><p className="mt-4 text-[10px] font-bold text-[#E7DFAE]">â— Em acompanhamento</p></div><div className="grid sm:grid-cols-3"><div className="border-b border-[#DDE2DB] px-5 py-4 sm:border-b-0 sm:border-r"><p className="text-[9px] font-bold uppercase tracking-[0.13em] text-[#637268]">01 Â· Base</p><p className="mt-2 text-[12px] font-bold text-[#294038]">{base}</p></div><div className="border-b border-[#DDE2DB] px-5 py-4 sm:border-b-0 sm:border-r"><p className="text-[9px] font-bold uppercase tracking-[0.13em] text-[#637268]">02 Â· Registro</p><p className="mt-2 text-[12px] font-bold text-[#294038]">{record}</p></div><div className="bg-[#F3F6F0] px-5 py-4"><p className="text-[9px] font-bold uppercase tracking-[0.13em] text-[#A9533A]">03 Â· PrÃ³ximo ato</p><p className="mt-2 text-[12px] font-bold leading-5 text-[#285A43]">{next}</p></div></div></div></section>;
}

function Footer() {
  return (
    <footer className="border-t border-[#E0E5DE] bg-[#FCFBF7] py-4">
      <div className="mx-auto max-w-[1360px] px-5 text-center text-[11px] text-[#5E6C64]">
...
function AccountabilityReportView() {
  ...
  return <div className="space-y-7"><section className="grid gap-7 border-b border-[#DDE2DB] pb-7 lg:grid-cols-[0.95fr_1.45fr]"><div><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-[#A9533A]">Leitura de gestÃ£o</p><h2 className="mt-2 font-editorial text-[29px] font-semibold leading-[1.05] tracking-[-0.05em] text-[#193B32]">Dados que sustentam a prestaÃ§Ã£o de contas.</h2><p className="mt-4 text-[13px] leading-6 text-[#657268]">Selecione o assunto e o perÃ­odo antes de registrar uma anÃ¡lise para consulta ou exportaÃ§Ã£o.</p></div><div className="grid gap-3 sm:grid-cols-2"><label><span className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#607068]">Conselho</span><div className="mt-2 flex h-11 items-center justify-between border border-[#D5DDD4] bg-[#FCFBF7] px-3 text-[12px] font-semibold text-[#526358]">Todos os conselhos <ChevronDown className="size-4" /></div></label><label><span className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#607068]">PerÃ­odo</span><div className="mt-2 flex h-11 items-center justify-between border border-[#D5DDD4] bg-[#FCFBF7] px-3 text-[12px] font-semibold text-[#526358]">Jan â€” Ago 2026 <CalendarDays className="size-4" /></div></label><div className="sm:col-span-2"><ActionButton onClick={() => toast.success("RelatÃ³rio gerado na visualizaÃ§Ã£o.")}><BarChart3 className="mr-2 size-4" />Gerar anÃ¡lise</ActionButton></div></div></section><section className="grid gap-7 lg:grid-cols-[225px_minmax(0,1fr)]"><nav className="flex gap-1 overflow-x-auto border-b border-[#DDE2DB] pb-2 lg:block lg:border-b-0 lg:border-r lg:pb-0 lg:pr-4">{reports.map((r, i) => <button key={r} onClick={() => setActive(r)} className={cn("flex shrink-0 items-center gap-3 px-3 py-2.5 text-left text-[12px] font-bold transition lg:w-full", active === r ? "bg-[#E8F0E8] text-[#285A43]" : "text-[#6F7B70] hover:bg-[#F2F4EF]")}><span className="text-[10px] text-[#A9533A]">0{i + 1}</span>{r}</button>)}</nav><article className="relative overflow-hidden border border-[#DDE2DB] bg-[#FCFBF7] p-5 pl-7 sm:p-6 sm:pl-9"><span className="absolute inset-y-0 left-0 w-1 bg-[#173F34]" /><div className="flex flex-wrap justify-between gap-3"><div><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#607068]">Caderno de evidÃªncias Â· 08/2026</p><h3 className="mt-1 font-editorial text-[24px] font-semibold tracking-[-0.04em] text-[#193B32]">{active} por perÃ­odo</h3></div><div className="text-right"><StatusPill tone="confirmed">Pronto para exportar</StatusPill><p className="mt-2 text-[9px] font-bold uppercase tracking-[0.12em] text-[#637268]">Protocolo 2026-08-REL</p></div></div><div className="mt-7 grid gap-px border border-[#DDE2DB] bg-[#DDE2DB] sm:grid-cols-3">{[["01 Â· Total", active === "PresenÃ§a" ? "88%" : "36"], ["02 Â· VariaÃ§Ã£o", "+12%"], ["03 Â· PerÃ­odo", "8 meses"]].map(([label, value]) => <div key={label} className="bg-[#F7F8F4] p-4"><p className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#607068]">{label}</p><p className="mt-2 font-editorial text-[27px] font-semibold tracking-[-0.05em] text-[#193B32]">{value}</p></div>)}</div><div className="mt-7 border-y border-[#DDE2DB] bg-[#F8F9F5] px-4 pb-2 pt-5"><div className="mb-3 flex justify-between text-[9px] font-bold uppercase tracking-[0.13em] text-[#829082]"><span>Ritmo mensurado</span><span>Jan â†’ Ago</span></div><div className="flex h-32 items-end gap-2">{[34,55,42,75,58,89,66,96,74,86,61,84].map((h, i) => <div key={i} className="flex-1 bg-[#D6E2D5] transition hover:bg-[#285A43]" style={{ height: `${h}%` }} />)}</div></div><div className="mt-4 flex flex-wrap justify-between gap-3 text-[11px] text-[#6E7A70]"><span>Fonte: registros do ConselhoOS Â· dados demonstrativos.</span><button onClick={() => toast.message("A exportaÃ§Ã£o em PDF serÃ¡ processada apÃ³s a conexÃ£o do backend.")} className="inline-flex items-center gap-1.5 font-bold text-[#285A43]">Exportar PDF <Download className="size-3.5" /></button></div></article></section></div>;
}
    ...
    <div className="space-y-8">
      <section className="grid overflow-hidden border border-[#DDE2DB] bg-[#FCFBF7] lg:grid-cols-[1fr_286px]">
        <div className="p-6 sm:p-7">
          <div className="flex items-center justify-between gap-4"><StatusPill tone="confirmed">Ciclo ativo</StatusPill><span className="text-[11px] font-medium text-[#607068]">Quinta-feira, 28 de agosto</span></div>
        ...
        <aside className="relative overflow-hidden bg-[#173F34] p-6 text-white">
          ...
          <p className="relative text-[10px] font-bold uppercase tracking-[0.16em] text-[#C9D7C9]">PrÃ³ximo marco</p>
          <div className="relative mt-7"><div className="font-editorial text-[47px] font-semibold leading-none tracking-[-0.07em]">14:00</div><p className="mt-2 text-[13px] font-semibold">24Âª ReuniÃ£o OrdinÃ¡ria</p><p className="mt-1 max-w-[210px] text-[12px] leading-5 text-[#DCE9DD]">Conselho Municipal de SaÃºde Â· Sala PlenÃ¡ria</p></div>
          <button onClick={() => setLocation("/reunioes")} className="relative mt-8 inline-flex items-center gap-2 text-[12px] font-bold text-[#EAD99B] hover:text-white">Abrir preparaÃ§Ã£o <ArrowUpRight className="size-3.5" /></button>
        </aside>
      </section>
      ...
      <section className="grid gap-8 xl:grid-cols-[minmax(0,1.55fr)_minmax(300px,0.8fr)]">
        <article className="border-b border-[#DDE2DB] pb-1">
          <SectionHeading index="01" label="Ritmo institucional" title="Reuniões realizadas" action={<div className="flex items-center gap-1 rounded-full border border-[#DDE2DB] bg-[#FCFBF7] p-0.5"><button onClick={() => setRange("6m")} className={cn("rounded-full px-2.5 py-1 text-[10px] font-bold transition", range === "6m" ? "bg-[#173F34] text-white" : "text-[#637066] hover:bg-[#E0E7DE]")}>6 meses</button><button onClick={() => setRange("12m")} className={cn("rounded-full px-2.5 py-1 text-[10px] font-bold transition", range === "12m" ? "bg-[#173F34] text-white" : "text-[#637066] hover:bg-[#E0E7DE]")}>12 meses</button><a href="/relatorios" onClick={(e) => { e.preventDefault(); setLocation("/relatorios"); }} className="ml-1 hidden items-center gap-1 px-1 text-[11px] font-bold text-[#285A43] hover:text-[#A9533A] sm:inline-flex">Relatório <ArrowUpRight className="size-3" /></a></div>} />
  ...
  return (
    <Empty className="min-h-[420px] rounded-none border-[#DDE2DB] bg-[#FCFBF7] p-7">
      <EmptyHeader>
        ...
        <p className="mt-1 text-[10px] font-bold uppercase tracking-[0.15em] text-[#A9533A]">Módulo em preparação</p>
      ...
      <EmptyContent>
        <div className="flex flex-wrap items-center justify-center gap-3">
          <Button onClick={() => toast.success("Módulo registrado no roteiro de implementação do MVP.")} className="h-10 rounded-xl bg-[#173F34] px-4 text-[12px] font-semibold text-white hover:bg-[#245446]">
            <Sparkles className="mr-2 size-3.5" />Registrar prioridade
          </Button>
          <Button onClick={() => toast.message("A integração com o banco de dados será o próximo passo.")} variant="outline" className="h-10 rounded-xl border-[#CBD4CA] bg-white text-[12px] font-bold text-[#285A43] hover:bg-[#EAF1E9]">
    ...
    <div className="flex h-full flex-col">
      ...
      <nav aria-label="Navegação principal" className="sidebar-scroll mt-8 flex-1 overflow-y-auto pr-1">
        {navGroups.map((group) => (
          <div key={group.label} className="mb-6">
            <p className="mb-2 px-2 text-[9px] font-bold uppercase tracking-[0.15em] text-[#67756C]">{group.label}</p>
              ...
              {group.items.map((item) => {
                ...
                return (
                  <a
                    ...
                    className={cn("flex w-full items-center gap-3 rounded-lg px-2.5 py-2.5 text-left text-[12px] font-semibold transition focus-visible:outline-none", isActive ? "bg-[#E8F0E8] text-[#285A43]" : "text-[#68756B] hover:bg-[#F1F3EE] hover:text-[#294038]")}
                  >
                    <Icon className={cn("size-4", isActive ? "text-[#A9533A]" : "text-[#607068]")} />
                    {item.label}
                    {item.key === "encaminhamentos" && (
                      <span className="ml-auto grid size-5 place-items-center rounded-full bg-[#F2E3DB] text-[9px] text-[#A9533A]">9</span>
                    )}
                  </a>
                );
              })}
      ...
      <div className="border-t border-[#E0E5DE] pt-4">
        <a
          ...
          className="flex w-full items-center gap-3 rounded-lg px-2.5 py-2.5 text-[12px] font-semibold text-[#68756B] transition hover:bg-[#F1F3EE] focus-visible:outline-none"
        >
        ...
        <div className="mt-3 flex items-center gap-2.5 border border-[#E0E5DE] bg-[#F7F8F4] p-2.5">
          <Avatar className="size-8 rounded-lg">
            <AvatarFallback className="rounded-lg bg-[#D5E2D3] text-[10px] font-bold text-[#285A43]">{profile?.full_name?.charAt(0) ?? user?.email?.charAt(0) ?? "U"}</AvatarFallback>
          </Avatar>
          <div className="min-w-0">
            <p className="truncate text-[11px] font-bold text-[#294038]">{profile?.full_name ?? user?.email ?? "Usuário"}</p>
            <p className="truncate text-[9px] text-[#5E6C64]">{profile?.role ?? "membro"}</p>
          </div>
        </div>
        <button onClick={async () => { await signOut(); setLocation("/"); }} className="mt-2 flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-[11px] font-bold text-[#A9533A] transition hover:bg-[#FDF3ED] focus-visible:outline-none">
        ...
        <div className="mx-auto max-w-[1360px] px-4 py-4 sm:px-6 lg:px-8">
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3 border-b border-[#DDE2DB] pb-4">
            <div className="flex items-center gap-3">
              ...
              <nav aria-label="Trilha" className="flex items-center gap-1.5 text-[11px] font-semibold text-[#607068]">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setPaletteOpen(true)} className="inline-flex h-9 items-center gap-2 rounded-lg border border-[#D5DDD4] bg-[#FCFBF7] px-3 text-[12px] font-semibold text-[#536358] transition hover:border-[#285A43] hover:text-[#285A43] focus-visible:outline-none">
                <Search className="size-3.5" />Buscar <kbd className="hidden rounded border border-[#DDE2DB] bg-[#F1F3EE] px-1 text-[9px] text-[#5E6C64] sm:inline">Ctrl K</kbd>
              </button>
